...
def replicate_rows(file_path: str, sheet_name: str, output_path: str = None, start_row: int = 2083, end_row: int = None, copies: int = 11, seq_col: int = 4, desc_col: int = 10):
    """Replicate each row in [start_row..end_row] `copies` times.

    For each original row, parse the sequence value in `seq_col` as hex (preferred).
    Then write `copies` rows with sequence values base..base+copies-1 (formatted in hex),
    and replace the first numeric occurrence in `desc_col` with the decimal sequence value.
    Rows before start_row and after end_row are copied unchanged.
    """
    if output_path is None:
        output_path = file_path.replace('.xlsx', '') + f'_replicated_{copies}.xlsx'
    ...
    for r in range(start_row, end_row + 1):
        # capture source row values
        src_vals = [ws.cell(row=r, column=c).value for c in range(1, max_col + 1)]

        # determine base sequence for this row
        base_val, base_base, base_meta = try_parse_int(src_vals[seq_col - 1])
        if base_val is None:
        ...
        for k in range(copies):
            ...
            for c in range(1, max_col + 1):
                if c == seq_col:
                    new_row.append(format_int(seq_val, base_base, base_meta))
                elif c == desc_col:
                    template = src_vals[c - 1]
                    if template is None:
                        template = ''
                    if isinstance(template, str):
                        ...
                        if m:
                            start, end = m.span()
                            new_desc = template[:start] + str(seq_val) + template[end:]
                        else:
                            new_desc = template
                        new_row.append(new_desc)
                    else:
                        new_row.append(template)
                else:
                    new_row.append(src_vals[c - 1])

            ws2.append(new_row)

    # copy rows after end_row
    for r in range(end_row + 1, ws.max_row + 1):
        vals = [ws.cell(row=r, column=c).value for c in range(1, max_col + 1)]
        ws2.append(vals)

    wb2.save(output_path)
    print(f"Replicated rows {start_row}-{end_row} x{copies}; saved to {output_path}")


if __name__ == '__main__':
    file_path = 'data.xlsx'
    sheet_name = 'Sheet1'
    out = 'data_replicated.xlsx'
    # replicate rows from 2083 to end, 11 copies (01..0B) per row, seq in col D (4), desc in col J (10)
    replicate_rows(file_path, sheet_name, out, start_row=2083, end_row=None, copies=11, seq_col=4, desc_col=10)
