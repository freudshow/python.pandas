...
def expand_ellipsis_rows(file_path: str, sheet_name: str, output_path: str = None, start_row: int = 1, seq_col: int = 3, desc_col: int = 10):
    ...
    # Expand ranges for the sequence column (seq_col) where a concrete
        ...
        for i, v in enumerate(seq):
            ...
            for c in range(1, max_col + 1):
                if c == seq_col:
                    ...
                    # for description column, replace the first number occurrence with decimal v
                    ...
                    if c == desc_col:
                        ...
                        if isinstance(template, str) and template:
                            ...
                            if m:
                                start, end = m.span()
                                new_desc = template[:start] + str(v) + template[end:]
...
if __name__ == '__main__':
    # minimal entrypoint - adjust these values as needed or call from another script
    file_path = 'data.xlsx'
    sheet_name = 'Sheet1'
    out = 'data_expanded.xlsx'
    # start at row 2083, sequence column D (4), description column J (10)
    expand_ellipsis_rows(file_path, sheet_name, out, start_row=2083, seq_col=4, desc_col=10)
